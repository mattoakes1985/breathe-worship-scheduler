...
Second hands-on feedback round shipped: bidirectional service-status control in the rota builder; song library fully reconciled against 5 years of monthly rota tabs (33 recovered songs → 202 total); root-caused the "changes not showing" report to the dev service worker cache-first-ing Vite modules (SW now registers in production only, dev self-cleans); zero-serves dashboard copy fixed; back buttons on all drill-in pages; Female/Male key labels spelt out (F collided with the key F); mobile quick links added (lead/admin/songs/preferences were unreachable on phones). Awaiting verify + commit.

### [2026-07-06 00:15] Fable 5 — status

v1.1 change batch built (see MEMORY.md entry for the full list): migrations 00006–00007 applied live (linked services, preference_rank, song male_key/time_signature, set_my_role_preference RPC), 169 songs imported from the team spreadsheet, engine upgraded with instrument-preference tier + 4 new tests (20 total), and frontend changes across RotaBuilder (alternatives picker, link/copy-rota controls), ServicePlanner (new — lead song/running-order editing with F/M/custom key picker), ServicesAdmin (bulk actions), Availability (locked answers stay visible), TeamLeadDashboard (posted swaps surfaced), Schedule (date-range CSV export), Preferences (instrument ranking). Types patched by hand for the new columns. Verify run on Matt's machine: **GREEN** (lint clean, 20/20 tests, build OK — one missed RPC type fixed en route). Dashboard attention banner also added at Matt's request. Awaiting commit + §13 click-through + §14 go/no-go.
